...
public class Validator<T, E: Error> {

    private var rules: [(T?) -> Result<Void, E>] = []

    func validate(_ value: T?) -> Result<T, E> {
        if case .failure(let error) = rules.map({ $0(value) }).first(where: { $0.isSuccess == false }) {
            return .failure(error)
        } else {
            guard let value = value else {
                fatalError("Please use .required for this validaton!!!!")
            }
...
extension Validator where T == String {
    ...
    func required(_ error: E) -> Validator {
        self.rules.append { value in
            (value != nil).map(error)
        }
        return self
    }
    
    func length(_ length: Int, _ error: E, nilResponse: Bool = false) -> Validator {
        self.rules.append { value in
            guard let value = value else {
    ...
    func isEqual(_ string: String?, _ error: E, nilResponse: Bool = false) -> Validator {
        self.rules.append { value in
            guard let value = value else {
                return nilResponse.map(error)
            }
    ...
    func text(_ error: E, nilResponse: Bool = false) -> Validator {
        self.rules.append {
            self.regex(value: $0, Regex.AlphanumericRegex, error, nilResponse)
        }
        return self
    }
    
    func email(_ error: E, nilResponse: Bool = false) -> Validator {
        self.rules.append {
            self.regex(value: $0, Regex.EmailRegex, error, nilResponse)
        }
        return self
    }
    
    func password(_ error: E, nilResponse: Bool = false) -> Validator {
        self.rules.append {
            self.regex(value: $0, Regex.Password, error, nilResponse)
        }
        return self
    }
    
    private func regex(value: String?, _ regex: String, _ error: E, _ nilResponse: Bool = false) -> Result<Void, E> {
        guard let value = value else {
            return nilResponse.map(error)
        }
